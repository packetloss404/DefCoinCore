...
The Defcoin Core daemon can be configured to act as a trusted "border
router", implementing the defcoin wire protocol and relay, making
...
The ZeroMQ feature in Defcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building defcoind:
...
For instance:
    ...
    $ defcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/defcoind.tx.raw \
...
These options can also be provided in defcoin.conf.
...
From the perspective of defcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into defcoind directly. Furthermore, no information is
...
using. Defcoind appends an up-counting sequence number to each
